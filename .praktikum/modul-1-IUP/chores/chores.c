...
int main(int argc, char const *argv[])
{
    ...
    // first line input
    int energy, nJumlah;
    scanf("%d %d", &energy, &nJumlah);

    // second input
    int priority;
    char str [65];
    for (int i = 0; i < nJumlah; i++) {
        scanf("%d", &priority);
        scanf("%s", str);
        // masukin stringnya ke suatu string
        pqueue_push(&myPque, priority);
    }
    int flag;
    flag = 0;
    // cetak isi pque
    while (0 < energy) {
        energy = energy - pqueue_top(&myPque);
        // printf("energy : %d \n", energy);
        if (energy < 0 && flag == 0) {
