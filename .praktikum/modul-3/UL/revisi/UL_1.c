...
void getStatus(AVLNode *root, int ind, int key[MAX][MAX], int strength[MAX][MAX], int sum[MAX], int last[MAX])
{
    if (root)
    {
        ...
        sum[ind] += root->strength; // total power musuh
        last[ind]++;
        getStatus(root->right, ind + 1, key, strength, sum, last);
    }
}

bool checkWin(AVLNode *root, int *stage, char *nama_Planet, ull powerUltraMan)
{
    int key[MAX][MAX], strength[MAX][MAX], sum[MAX], last[MAX];
    memset(sum, 0, sizeof(sum));
    memset(last, 0, sizeof(last));
    getStatus(root, 0, key, strength, sum, last); // musuh
...
int main()
{
    ...
    int nJumlah, keyMusuh, damageMusuh, stage = 1;
    ull powerUltraMan;
    char namaPlanet[100];
    scanf("%d %llu %s", &nJumlah, &powerUltraMan, namaPlanet);
    ...
    getMaxStage(myAvl.root); // buat dapetin max stage

    printf("Ultramen lapet(Power: %llu)\nPlanet: %s\n---------------\n", powerUltraMan, namaPlanet);
    ...
    {
        if (stage == maxStage) // kalau stage yg di lewati == max stage
