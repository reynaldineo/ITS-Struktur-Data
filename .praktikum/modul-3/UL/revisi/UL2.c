...
bool canPass(AVLNode *root, int *stage, char *nama_Planet, int powerUltraMan)
{
    ...
    {
        if (i == limit - 1) // kalau udh perulangan terakhir alias masuk Final stage
        {
            printf("Final Stage:\n");
...
int main()
{
    ...
    if (canPass(myAvl.root, &stage, namaPlanet, powerUltraMan)) // bisa menang sampai akhir ga?
        printf("\nResult:\nUltramen Lapet lolos dari Planet %s", namaPlanet);
    else
    {
        if (stage == limit) // kalau stage yg di lewati == limit stage
