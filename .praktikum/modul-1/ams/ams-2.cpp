...
int main(){
  ...
  while (q--) {
    ...
    priority_queue<dataStruk, vector<dataStruk>, priorityCompare> pqueueTemp;
    pqueueTemp.push(dataStruk{id, skor});

    while (!pqueueTemp.empty()) {
      if (id == pqueueTemp.top()._id) {
        int skorTemp = skor + pqueueTemp.top()._skor;
      }
      pqueueTemp.pop();
    }
    pqueueTemp.push(dataStruk{id, skor});
